			...
			for (int t = 0; t < T + markovLag; t++) {
				...
				for (int i = 0; i < n; i++) {
					...
					if (attributeName != null)
						...
						sb.append("[label=\"X" + i);
					if(t==(T + markovLag-1)){
						sb.append("[" + t + "]\", color = DodgerBlue];" + ls);
					}
					else{
						sb.append("[" + t + "]\"];" + ls);
					}
...
//////////////////////////// GENERATE A DBN FOR TESTING PURPOSES, ARTAFICIAL DATA AND OUTLIER GENERATION
		
		
//		X1[0]=>X3[0],X2[0]=>X2[1],X2[1]=>X3[1]
//		com X1, X2 e X3 variáveis binarias
//		estacionária e com T=3
		...
		List<Edge> prior = Arrays.asList(e02);
		BayesNet b0 = new BayesNet(a, prior);
		b0.generateParameters();

		//** Criar a transition network:

		List<Edge> inter = Arrays.asList(e11);

		List<Edge> intra = Arrays.asList(e12);

		BayesNet bt = new BayesNet(a, intra, inter);

		bt.generateParameters();

		//** Para gerar a DBN estacionária com prior network b0 e transition
		//network bt, considerando T=3:

		DynamicBayesNet dbn1 = new DynamicBayesNet(a, b0, Arrays.asList(bt,bt,bt));

		//** Para gerar 5000 observações:

		Observations o = dbn1.generateObservations(5000);

		//** Para escrever num ficheiro "output":
		
		System.out.println("Writing Artaficial Data in file");

		o.writeToFile("artaficial_data");	
